...
async def create_session(req: SessionCreateRequest, db: AsyncSession = Depends(get_db)):
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        try:
            await client.post(
                f"http://localhost:8002/session/{session.id}/contract",
                json={
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        try:
            await client.post(
                f"http://localhost:8003/session/{session.id}/init",
                json={
                    "teacher_user_id": req.teacher_user_id,
                    "learner_user_id": req.learner_user_id,
                },
...
async def start_session(barter_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(BarterSession).where(BarterSession.id == barter_id)
    )
    ...
    if session.status == "active":
        return {"barter_id": session.id, "started_at": session.started_at.isoformat(), "status": session.status}
...
async def confirm_session(
    barter_id: int, req: ConfirmRequest, db: AsyncSession = Depends(get_db)
):
    result = await db.execute(
        select(BarterSession).where(BarterSession.id == barter_id)
    )
    ...
    if both_confirmed:
        ...
        # Notify Person 2 (audio pipeline) — end session
        async with httpx.AsyncClient(timeout=5.0) as client:
            try:
                await client.post(
                    f"http://localhost:8001/session/{barter_id}/end"
                )
            except Exception:
                pass
...
async def session_status(barter_id: int, db: AsyncSession = Depends(get_db)):
    ...
    contract_result = await db.execute(
        select(SessionContract).where(
            SessionContract.barter_session_id == barter_id
        )
    )
    ...
    if session.started_at:
        ...
        elapsed_minutes = round(
            (end_time - session.started_at).total_seconds() / 60, 1
        )
...
async def log_window_result(req: WindowResultRequest, db: AsyncSession = Depends(get_db)):
    ...
    await manager.broadcast(req.barter_id, {
        "type": "window",
...
async def receive_engagement_summary(
    barter_id: int, req: dict, db: AsyncSession = Depends(get_db)
):
...
async def update_trust(barter_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if not verdict:
        raise HTTPException(
            status_code=404, detail="Verdict not found. Generate verdict first."
        )
...
async def check_video_frame(req: FrameCheckRequest):
    ...
    if result:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            try:
                await client.post(
                    "http://localhost:8003/safety/alert",
                    json={
                        "barter_id": req.barter_id,
                        "user_id": req.user_id,
                        "warning_type": "nsfw",

...
class User(Base):
    ...
    trust_score: Mapped[float] = mapped_column(Float, default=0.30, nullable=False)
...
class Verdict(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    barter_session_id: Mapped[int] = mapped_column(ForeignKey("barter_sessions.id"), unique=True, nullable=False)
